...
- **After merging anything, run the whole suite before pushing.** A clean merge
  ...
  parsed.
- **The suite runs on SQLite. The shops run on MySQL. Anything touching raw
  SQL gets run against MariaDB here before it ships.** This has now cost two
  outages, both with a fully green suite: `as lines` (LINES is reserved in

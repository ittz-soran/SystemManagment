...
@section('doc-title', __('Purchase return'))
...
@section('content')
    <div class="mb-3">
        ...
        <div class="fw-semibold">{{ $return->supplier->name }}</div>
        <div class="small">
            {{ __('Against purchase :document', ['document' => $return->purchase->document_no]) }}
        </div>
    </div>
    ...
    <table class="table table-sm">
        ...
        @foreach($return->items as $item)
            <tr>
                ...
                <td class="money">{{ money($item->creditTotal(), false) }}</td>
            </tr>
        @endforeach
        </tbody>
        <tfoot>
